...
export default class Pagination extends React.Component {
  ...
  renderPage = totalPage => {
    ...
    let prevClass = 'pagination-item pagination-item-prev pagination-item-active'
    let nextClass = 'pagination-item pagination-item-next pagination-item-active'
    let lastClass = 'pagination-item pagination-item-last pagination-item-active'
    if (current === 1) {
    ...
    const pages = []
    pages.push(<li className={prevClass} onClick={() => {
      this.handlePrev()
    }} key='prev'>{prevIcon}
    </li>)
    if (totalPage < 10) {
      ...
      for (let i = 1; i <= totalPage; i++) {
        ...
        pages.push(<li className={classNames} key={i} onClick={() => {
          this.pageClick(i)
        }}>{i}</li>)
      }
    } else {
      // 否则显示部分
      pages.push(<li className={current === 1 ? 'pagination-item pagination-item-active' : 'pagination-item'}
                     key={1} onClick={() => {
        this.pageClick(1)
      }}>1</li>)
      let pageLength = 0;
      ...
      for (let i = startPage; i < pageLength; i++) {
        if (i <= totalPage - 1 && i > 1) {
          pages.push(
            <li className={current === i ? 'pagination-item pagination-item-active' : 'pagination-item'}
                key={i}
      ...
      if (totalPage - startPage >= groupCount + 1) {
        pages.push(<li className="pagination-item" key={-2}>···</li>)
      }
      pages.push(<li
        className={current === totalPage ? 'pagination-item pagination-item-active' : 'pagination-item'}
        key={totalPage} onClick={() => {
        this.pageClick(totalPage)
      }}>{totalPage}</li>)
    }
    pages.push(<li className={nextClass} key='next' onClick={() => {
      this.handleNext()
    }}>{nextIcon}</li>)
    pages.push(<li className={lastClass} key='last' onClick={() => {
      this.handleLast()
    }}>末页</li>)
    return pages
  }
        ...
        <ul className='pagination-content'>
          {showTotal ?
            (typeof showTotal !== 'function' ?
              <span
                className="total-counter">共{totalPage}
                页
              </span> :
              showTotal(Math.ceil(total / pageSize), (current - 1) * pageSize + 1, current === totalPage ?
